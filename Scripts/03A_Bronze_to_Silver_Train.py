...
def setup_logging() -> logging.Logger:
    ...
    log_file = logs_dir / f"03_Bronze_to_Silver_{dt_mark}.log"
...
def create_silver_train_table(duckdb_name: str = "assignment.duckdb") -> None:
    """
    Create silver schema train table by joining bronze tables
    """
    logger = logging.getLogger('BronzeToSilver')
    ...
    try:
        ...
        query = """
        CREATE OR REPLACE TABLE silver.train AS
        SELECT
            sr.*,
            io.* EXCLUDE (Client),
            pa.* EXCLUDE (Client),
            sd.* EXCLUDE (Client)
        FROM bronze.sales_revenues sr
        LEFT JOIN bronze.inflow_outflow io ON sr.Client = io.Client
        LEFT JOIN bronze.products_actbalance pa ON sr.Client = pa.Client
        LEFT JOIN bronze.soc_dem sd ON sr.Client = sd.Client
        """
        ...
        logger.info("Executing JOIN query to create silver.train table...")
        conn.execute(query)
        logger.info("Successfully created silver.train table with JOINs")
        ...
        columns_result = conn.execute("""
            SELECT column_name, data_type
            FROM information_schema.columns
            WHERE table_schema = 'silver' AND table_name = 'train'
            ORDER BY ordinal_position
        """).fetchall()
        ...
        result = conn.execute("SELECT COUNT(*) as row_count FROM silver.train").fetchone()
        logger.info(f"Verified: {result[0]} rows in silver.train table")
        ...
        # Show first few rows
        logger.info("First 3 rows of silver.train:")
        preview = conn.execute("SELECT * FROM silver.train LIMIT 3").fetchdf()
        logger.info(f"\n{preview}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        create_silver_train_table(duckdb_name="assignment.duckdb")
